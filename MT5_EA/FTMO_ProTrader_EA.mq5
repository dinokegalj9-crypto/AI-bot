...
input int    InpEntryMode          = 0;       // Entry strategy: 0=pullback (FX), 1=breakout (gold/indices)
input int    InpMinSignals         = 4;       // [Mode 0] Min confluence signals to trade (3-5)
input int    InpBreakoutLookback   = 20;      // [Mode 1] Prior bars for range breakout
input int    InpADXPeriod          = 14;      // ADX period (trend strength)
input double InpADXMin             = 20.0;    // Min ADX to confirm trending regime
input double InpPullbackRSI        = 50.0;    // [Mode 0] RSI level defining a pullback dip
